...
With `display.enabled = true` (the default in `~/.config/actual/gateway.toml`), gateway has already created display :99 on startup and auto-attached an xpra viewer. In this case, **skip display creation** — the display already exists and all tools are already routing to :99.
